...
def main(args, ITE=1):
    ...
    train_loaderA, test_loaderA = get_split(dataset_name =  args.dataset, batch_size=args.batch_size, train = traindataset, test = testdataset, model_name = 'full', shuffle=True)
    ...
    weight_dir = f"{os.getcwd()}/saves/{args.arch_type}/{args.dataset}/init_for_lth.pth"
    
    
    ## required for weight rewinding
    model0 = modelselect(args, output_class=args.output_class) ## initialize a model with random weights
    ...
    #     torch.save(model0.state_dict(), weight_dir)      
        ...
        print("\nPLEASE COPY \"init_for_lth\" from colt transfers directory to this directory")
        exit()

    if not resume:             
        model1 = modelselect(args, output_class=args.output_class)
        model1.load_state_dict(initial_state_dict)
        model1.to(device)

#         model2 = modelselect(args, output_class=args.output_class)
...
#         compare_models(model1,model2)
        ...
        cp = torch.load(f"{os.getcwd()}/checkpoints/{args.arch_type}/{args.dataset}/{args.prune_type}/checkpoint_model_A.pth")
    ...
    while (prune_rate < args.final_prune_rate):
        ...
        if pruning_round == 0:
            ...
            if pt == 'lth':           
                ...
                checkdir(f"{os.getcwd()}/saves/{args.arch_type}/{args.dataset}/{pt}/full")
                # checkdir(f"{os.getcwd()}/saves/{args.arch_type}/{args.dataset}/{pt}/B")
                torch.save(model1.state_dict(),f"{os.getcwd()}/saves/{args.arch_type}/{args.dataset}/{pt}/full/{prune_rate:.2f}_model_{pt}.pth")
...
#                 torch.save(model1.state_dict(),f"{os.getcwd()}/saves/{args.arch_type}/{args.dataset}/{pt}/{prune_rate:.2f}_model_{pt}.pth")
            ...
            model1, all_loss1, all_accuracy1, accuracy1, comp1, best_accuracy1, best_val_loss1 = training(model=model1, args = args, train_loader=train_loaderA, 
                                                                                                    test_loader = test_loaderA, model_type="full", bias=bias)
...
#                 torch.save(model1.state_dict(),f"{os.getcwd()}/saves/{args.arch_type}/{args.dataset}/{pt}/{prune_rate:.2f}_model_{pt}.pth")
        ...
        torch.save(checkpoint, f"{os.getcwd()}/checkpoints/{args.arch_type}/{args.dataset}/{args.prune_type}/checkpoint_model_A.pth")  
        
        # Dump Plot values
        checkdir(f"{os.getcwd()}/dumps/{pt}/{args.arch_type}/{args.dataset}/full")
        # checkdir(f"{os.getcwd()}/dumps/{pt}/{args.arch_type}/{args.dataset}/B")
        all_loss1.dump(f"{os.getcwd()}/dumps/{pt}/{args.arch_type}/{args.dataset}/full/{pt}_all_loss_{comp1}.dat")
        all_accuracy1.dump(f"{os.getcwd()}/dumps/{pt}/{args.arch_type}/{args.dataset}/full/{pt}_all_accuracy_{comp1}.dat")
        ...
        np_compa.dump(f"{os.getcwd()}/dumps/{pt}/{args.arch_type}/{args.dataset}/full/{pt}_compression.dat")
        np_bestacc1.dump(f"{os.getcwd()}/dumps/{pt}/{args.arch_type}/{args.dataset}/full/{pt}_bestaccuracy.dat")
    ...
    checkdir(f"{os.getcwd()}/plots/{pt}/{args.arch_type}/{args.dataset}/full")
    plt.savefig(f"{os.getcwd()}/plots/{pt}/{args.arch_type}/{args.dataset}/full/{pt}_AccuracyVsWeights.png", dpi=1200, bbox_inches='tight') 
    plt.close()   
...
if __name__=="__main__":   
    ...
    parser.add_argument("--prune_type", default="lth", type=str, help="lth")
    ...
    parser.add_argument("--output_class", default=100, type=int, help="Output classes for the model")
    ...
    parser.add_argument("--conv_prune_ratio", default=0.20, type=float, help="Conv layer pruning proportion")
